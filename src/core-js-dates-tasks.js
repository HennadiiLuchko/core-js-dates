...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const timeDiff = new Date(dateEnd).getTime() - new Date(dateStart).getTime();
  return Math.floor(timeDiff / dayInMs) + 1;
}
...
function isDateInPeriod(date, period) {
  ...
  const startRange = new Date(period.start);
  const endRange = new Date(period.end);
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (curDate <= endDate) {
    for (let i = 0; i < countWorkDays && curDate <= endDate; i += 1) {
      res.push(curDate.toJSON());
      curDate.setDate(curDate.getDate() + 1);
    }

    for (let i = 0; i < countOffDays && curDate <= endDate; i += 1) {
      curDate.setDate(curDate.getDate() + 1);
    }
  }

  return res.map((el) => el.slice(0, 10).split('-').reverse().join('-'));
}
